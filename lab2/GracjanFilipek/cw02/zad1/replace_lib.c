...
int main(int argc, char **argv)
{
    ...
    int chars_read = fread(buffer, sizeof(char), BUFFER_SIZE, in_file);
    ...
    while (chars_read > 0)
    {
        ...
        fwrite(buffer, sizeof(char), chars_read, out_file);

        chars_read = fread(buffer, sizeof(char), BUFFER_SIZE, in_file);
        curr_char = buffer;
    }
